<template>
    <div class="about pb-6">
        <v-container grid-list-xs>   
            ...
            <v-row>
                <v-col cols="12">
                    <v-data-table :headers="headers" :items="sos" :search="search" sort-by="correos" class="elevation-1 custom-box" item-key="id"
                        :footer-props="{
                    ...
                    >
                        <template v-slot:top>
                            <v-toolbar flat>
                                <v-toolbar-title>Servicios de Origen</v-toolbar-title>
                                    ...
                                    <v-card>
                                        ...
                                        <v-card-text class="pt-2">
                                            <v-form ref="form" v-model="valid" lazy-validation>
                                                ...
                                                ></v-text-field>
                                                <v-text-field 
                                                    filled v-model="editedItem.service" label="Servicio de Origen" 
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: "Servicio de Origen", value: "service" },
        ...
        rules: {
            ...
            service: [
                (v) => !!v || "El servicio de origen es requerido",
                (v) => (v && v.length >= 6) || "El servicio de origen debe tener mínimo 6 caracteres",
            ],
        }
    }),
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? "Nuevo Servicio de Origen" : "Editar Servicio de Origen";
        },
    },
    watch: {
        dialog(val) {
    ...
    methods: {
        ...
        initialize() {
            axios
                .get("http://localhost:3000/api/origen")
                .then((response) => {
                    console.log("Response: ", response);
                    this.sos = response.data.data;
                })
                .catch((error) => {
                    console.log(error);
                });
        ...
        deleteItemConfirm() {
            axios
                .delete(`http://localhost:3000/api/origen/${this.editedItem.id}`)
                .then((response) => {
                    console.log("Response: ", response);
            ...
            if (this.editedItem.name === "" || this.editedItem.email === "" || this.editedItem.service === "") {
                ...
                if(this.$refs.form.validate() === true) {
                    if (this.editedIndex > -1) {
                        axios
                            .put("http://localhost:3000/api/origen/" + this.editedItem.id, this.editedItem)
                            .then((response) => {
                                console.log("Response: ", response);
                                this.$store.dispatch("showSuccessMessage", response.data.message);
                                this.initialize();
                        ...
                        axios
                            .post("http://localhost:3000/api/origen", this.editedItem)
                            .then((response) => {
                                console.log("Response: ", response);
                                this.$store.dispatch("showSuccessMessage", response.data.message);
                                this.initialize();
